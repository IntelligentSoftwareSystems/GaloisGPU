...
__global__ void preprocess(CSRGraphTex graph, unsigned int * valid_edges)
{
  unsigned tid = TID_1D;
...
__device__ unsigned int intersect(CSRGraphTex graph, index_type u, index_type v, unsigned int * valid_edges)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
__global__ void count_triangles(CSRGraphTex graph, unsigned int * valid_edges, int * count)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
