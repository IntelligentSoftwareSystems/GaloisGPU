/** Single source shortest paths -*- CUDA -*-
 * @file
 * @section License
 *
...
int main(int argc, char *argv[]) {
	...
	cudaMemcpy(hdist, dist, graph.nnodes * sizeof(foru), cudaMemcpyDeviceToHost);
	totalcommu += graph.nnodes * sizeof(foru);
	endtime = rtclock();
	
	printf("\truntime = %.3lf ms iterations = %d communication = %.3lf MB.\n", 1000 * (endtime - starttime), iteration, totalcommu * 1.0 / 1000000);
