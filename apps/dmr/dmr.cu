...
__global__ 
__launch_bounds__ (REFINE_BLKSIZE)
void refine(Mesh mesh, int debg, uint *nnodes, uint *nelements, GlobalBarrier gb, Worklist2 wl, Worklist2 owl)
{
  int id = threadIdx.x + blockDim.x * blockIdx.x;
  int threads = blockDim.x * gridDim.x;
...
__global__
__launch_bounds__ (CHECK_TRIANGLES_BLKSIZE)
void check_triangles(Mesh mesh, uint *bad_triangles, Worklist2 wl, int start)
{
  ...
  bool push;
  typedef cub::BlockScan<int, CHECK_TRIANGLES_BLKSIZE> BlockScan;
...
void refine_mesh(ShMesh &mesh)
{
  ...
  const size_t RES_REFINE = maximum_residency(refine, REFINE_BLKSIZE, 0); 
  ...
  *(nbad.cpu_wr_ptr(true)) = 0;
  ...
  check_triangles<<<nSM, CHECK_TRIANGLES_BLKSIZE>>>(gmesh, nbad.gpu_wr_ptr(), *inwl, 0); 
  ...
  while(cnbad) {   
    ...
    refine<<<nSM * RES_REFINE, REFINE_BLKSIZE>>>(gmesh, 32, nnodes.gpu_wr_ptr(), nelements.gpu_wr_ptr(), gb, *inwl, *outwl);
    ...
    *(nbad.cpu_wr_ptr(true)) = 0;
    ...
    //inwl->reset();
    check_triangles<<<nSM, CHECK_TRIANGLES_BLKSIZE>>>(gmesh, nbad.gpu_wr_ptr(), *inwl, lastnelements); 
    //cnbad = *(nbad.cpu_rd_ptr());
...
int main(int argc, char *argv[]) {
  ...
  printf("CHECK_TRIANGLES_BLKSIZE %d\nREFINE_BLKSIZE %d\n", CHECK_TRIANGLES_BLKSIZE, REFINE_BLKSIZE);
